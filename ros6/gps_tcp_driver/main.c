...
//	clock_gettime(CLOCK_REALTIME, &start);
	...
	// set card for synchronized generator mode, GPS reference enabled Daylight Savings Time Disabled	
	  *((uint32*)(BASE1+0x118))=0x90000021; //x90 to address x11B produced 10 MHz reference signal output
	...
	// clear time compare event
	  ...
	  *((uint32*)(BASE1+0x128))=10;  //set rate synthesiser rate to a default of 100ms (10pps)
	  *((uint32*)(BASE1+0x12c))=0x0200ff00;  // x12e: event trigger on external event, falling edge 
		  ...
		  if (rval<0) {
		    ...
		    switch( datacode ){
				...
				case GPS_SET_TRIGGER_RATE:
  		                        if (verbose > 0) printf("Inside Set Rate: %c\n", datacode);
					rval=recv_data(msgsock,&displaystat.ratesynthrate,sizeof(int));
					if (configured && locked) {
                                          *((uint32*)(BASE1+0x128))=displaystat.ratesynthrate;  //set rate synthesiser rate
//					  *((uint32*)(BASE1+0x12c))|=0x00000200;  // load rate synth rate 
                                        }
                                        rval=send_data(msgsock, &msg, sizeof(struct DriverMsg));
  		                        if (verbose > 0) printf("Leaving Set Rate: %c\n", datacode);
					break;
